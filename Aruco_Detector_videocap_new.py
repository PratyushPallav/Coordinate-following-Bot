...
def doprocess(img):
    ...
    if ids is not None :
        ...
        for id in dict:
            ...
            distance = round(math.sqrt((y_center - current_coord[1])**2 + (current_coord[0] - x_center)**2))
            cv2.putText(frame, 'Distance', (50, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (200, 0, 200), 2)
            cv2.putText(frame, str(distance), (50, 220), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (200, 0, 200), 2)
            ...
            finalangle = diff
            command = ""
            if(diff>10 and distance>50):
                command = "right"
            elif (diff<-10 and distance>50):
                command = "left"
            elif (abs(diff)<=10 and distance>50):
                command = "forward"
            else:
                command = "stop"
            write_read(command)
            ...
            cv2.line(frame, (int(x_center), int(y_center)), (int(current_coord[0]), int(current_coord[1])), (255, 0, 0),
                     3)
            cv2.putText(frame, '-ve for Anti_Clk and +ve for Clk', (200, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                        (0, 0, 200), 2)
    return img
